...
//多位置图清空
function fileToClear(num){
    var imgName= "img"+(num+1);
    window.localStorage.setItem(imgName,'');
}
//批量转换测点位置图片
function fileToDataURL(file,num){
...
function imgsParse(files){
    ...
    for(i=0;i<files.length;i++){
        ...
        reader.onload = function(e) {
            imageList[i] = e.target.result;//图片数据
        };
    }
    return imageList;
}
//解析Excel---多个文件的没有测试
function excelParse(files){
...
$(function () {
    ...
    function storeBasicInfo(){
        var jianZhuLeiXing = get_radio_value("jzlx");
        var qiHouFenQu = get_radio_value("qhfq");
        //jianZhuDiDian
    ...
    function getSecondPageFormData(){
        ...
        //单位面积照明能耗
        var dwmjzmhn=$("#dwmjzmhn").val();
        //其他环境质量/其他能耗(Q/L)s评价
        //其他单位面积能耗
        var qtdwmjhn = $("#qtdwmjhn").val();
    }
    //监听上传按钮并保存数据
    //第一步按钮保存表单基础数据
    ...
    //建筑外观
    $("#uploadJzwg").change(function(e){
        var files = this.files;
        var f = files[0];
        var reader = new FileReader();
        reader.readAsDataURL(f);
        reader.onload = function(e) {
             var uploadJzwg = e.target.result;//图片数据
             window.localStorage.setItem("uploadJzwg",uploadJzwg);
        };
    });
    //插件自带存在问题
    //$("#uploadDxpm").on('fileloaded', function(event, file, previewId, index) {
    ...
    //    reader.readAsDataURL(f);
    //    reader.onload = function(e) {
    //        var uploadDxpm = e.target.result;//图片数据
    //        window.localStorage.setItem("uploadDxpm",uploadDxpm);
    //    };
    //});
    //典型平面
    $("#uploadDxpm").change(function(e){
        var files = this.files;
        var f = files[0];
        var reader = new FileReader();
        reader.readAsDataURL(f);
    ...
    $("#uploadWsdcd").change(function(e){
       ...
       /* var files = this.files;
        ...
        reader.readAsDataURL(f);
        reader.onload = function(e) {
            var uploadWsdcd = e.target.result;//图片数据
            window.localStorage.setItem("uploadWsdcd",uploadWsdcd);
        };*/
    });
    ...
    $("#step2").click(function(){
        ...
        //单位面积采暖能耗
        var dwmjcnnh=(parseFloat($("#dwmjcnnh").val())/jianZhuMianJi).toFixed(1);
        //单位面积空调能耗
        var dwmjktnh=(parseFloat($("#dwmjktnh").val())/jianZhuMianJi).toFixed(1);
        //单位面积照明能耗
        var dwmjzmhn=(parseFloat($("#dwmjzmhn").val())/jianZhuMianJi).toFixed(1);
        //其他环境质量/其他能耗(Q/L)s评价
        //其他单位面积能耗
        var qtdwmjhn =(parseFloat($("#qtdwmjhn").val())/jianZhuMianJi).toFixed(1);
        //单位面积总能耗
        var dwmjznh = dwmjcnnh+dwmjktnh+dwmjzmhn+qtdwmjhn;
        ...
        // 放到本地存储
        var result5X =0.4*resultTmp1+0.2*resultTmp2+0.1*resultTmp3+0.3*resultTmp4;
    ...
    $("#wsd1").change(function(e){
        ...
        reader.onload = function(e) {
            ...
            for(var i=0;i<dataList.length;i++){
                ...
                //日期范围内的温度就叠加
                if(compareDate( wdcnStart,handleDateTime(rowItem[keyNames[1]]) ) && compareDate( handleDateTime(rowItem[keyNames[1]]),wdcnEnd )){
                    sum = sum + parseFloat(rowItem[keyNames[2]]);
                    number=number+1;
                }
            }
            var aveWinterTemperature = sum/number;
            console.log(aveWinterTemperature);
            window.localStorage.setItem("aveWinterTemperature",aveWinterTemperature.toFixed(1));
            //输出反馈结果
            console.log(ipcRenderer.sendSync('wsdSummer-message', dataTrans)); // prints "pong"
        };
    });
    //温湿度数据2
